...
- **THEN** the system SHALL hash the password, persist a new User document in MongoDB, and switch the auth page to the Sign In tab
...
- **WHEN** a user who already has a valid `auth-token` cookie visits `/`
